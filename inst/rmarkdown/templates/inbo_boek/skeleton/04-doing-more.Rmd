...
## Corporate style blocks in both HTML and PDF {#S:htmlblocks}

Corporate style special blocks can be accomodated in both HTML and PDF, as below, by combining them in the RMarkdown file. By making use of the `echo` (TRUE or FALSE) chunk option of the block2 engine, you can suppress blocks that should not be displayed for a certain output format^[The automated way in which it is used below, will only work from within RStudio.]. Also, note that unique labels have been inserted in order to cross-reference a block. For example: the [example block](#unique_exampleblock_label).
