...
- code_folding:
  ...
  Specify `"none"` to display all code chunks. Specify `"hide"` or
  `"show"` to hide or show all R code chunks by default, and let readers
  toggle the states on browsers. See the *Code folding*
...
- `github-repo`: You can optionally set the `github` repo in
  `organisation/repository` format (e.g. `inbo/INBOmd`). This will add
  an 'edit' button to the source file on GitHub. Note that only people
  with write access can edit the files directly in the repository.
...
- `split_by`: How to name the HTML output files from the book:
  ...
  - `section+number` means the second-level headers. For
    `chapter+number` and `section+number`, the HTML file names will be
    determined by the header ID's, e.g. the file name for the first
    chapter with a chapter title `# Introduction` will be
    `1-introduction.html`. Note that you need to place at least one
    level 1 heading in `index.Rmd`. Otherwise you don't get an
    `index.html` file.
